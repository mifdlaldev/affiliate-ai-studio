"use server";

import { createServerClient } from "@/lib/supabase/server";
import { generateText } from "@/lib/ai/client";
import { extractJson } from "@/lib/ai/json";
import { checkAndIncrementUsage } from "@/lib/usage/limits";
import {
  MARKETPLACE_SYSTEM_PROMPT,
  buildMarketplacePrompt,
} from "@/lib/ai/prompts/marketplace";
import { generateMarketplaceSchema } from "@/lib/validation/marketplace";
import type { Json } from "@/lib/supabase/types";

/**
 * Structured output of the Marketplace Product Description Generator
 * flow. The shape mirrors the JSON schema inside
 * `MARKETPLACE_SYSTEM_PROMPT` / `buildMarketplacePrompt` and is
 * persisted into `generations.result` (JSONB) for history + re-use.
 *
 * Marketplace listings are unique in that they have a `title`
 * (headline), `shortDescription` (search snippet), `description` (full
 * body), `bulletPoints` (feature list), `tags` (SEO keywords), and
 * `cta` (closing line). None of the other generators produce this
 * exact shape.
 */
export interface MarketplaceResult {
  title: string;
  shortDescription: string;
  description: string;
  bulletPoints: string[];
  tags: string[];
  cta: string;
}

export type GenerateMarketplaceResult = {
  data?: MarketplaceResult;
  error?: string;
};

/**
 * Server Action that runs the full Marketplace Product Description
 * Generator pipeline for the signed-in user:
 *
 *   1. Verify the user is authenticated.
 *   2. Parse + Zod-validate the submitted form (productId, platform,
 *      style, length, includeSpecs, targetAudience).
 *   3. Atomically check + increment the monthly usage limit (50/month
 *      soft cap, enforced via the `increment_user_usage` Postgres RPC).
 *   4. Load the saved product (RLS already scopes to `user_id`).
 *   5. Build the prompt + call the text model in JSON mode.
 *   6. Parse the AI's JSON-object response into `MarketplaceResult`.
 *   7. Persist the generation into the `generations` table.
 *   8. Return the marketplace description to the client.
 *
 * Never throws — every failure path returns `{ error: string }` so the
 * client can surface a friendly toast.
 */
export async function generateMarketplaceDescription(
  formData: FormData,
): Promise<GenerateMarketplaceResult> {
  try {
    const supabase = await createServerClient();
    const {
      data: { user },
    } = await supabase.auth.getUser();
    if (!user) {
      return { error: "Anda harus login" };
    }

    const parsed = generateMarketplaceSchema.safeParse({
      productId: formData.get("productId"),
      platform: formData.get("platform"),
      style: formData.get("style"),
      length: formData.get("length"),
      includeSpecs: formData.get("includeSpecs") ?? false,
      targetAudience: formData.get("targetAudience"),
    });
    if (!parsed.success) {
      return {
        error: parsed.error.issues[0]?.message ?? "Input tidak valid",
      };
    }

    const {
      productId,
      platform,
      style,
      length,
      includeSpecs,
      targetAudience,
    } = parsed.data;

    // 1. Atomic usage limit check + increment.
    const usage = await checkAndIncrementUsage(user.id);
    if (!usage.allowed) {
      return {
        error: `Batas 50 generate/bulan tercapai. Sisa: ${usage.remaining}.`,
      };
    }

    // 2. Load the saved product (RLS scopes to `user_id`).
    const { data: product, error: productError } = await supabase
      .from("products")
      .select(
        "id, name, category, brand, price, target_market, usp, benefits",
      )
      .eq("id", productId)
      .single();

    if (productError || !product) {
      return { error: "Produk tidak ditemukan" };
    }

    // 3. Build prompt + call the model in JSON mode.
    const prompt = buildMarketplacePrompt({
      product,
      platform,
      style,
      length,
      includeSpecs,
      targetAudience,
    });

    const result = await generateText({
      systemPrompt: MARKETPLACE_SYSTEM_PROMPT,
      prompt,
      jsonMode: true,
    });

    // 4. Parse the JSON-object response.
    let marketplace: MarketplaceResult;
    try {
      marketplace = extractJson<MarketplaceResult>(result.content);
    } catch (parseErr) {
      console.error(
        "generateMarketplaceDescription: failed to parse AI JSON",
        parseErr,
        result.content,
      );
      return { error: "AI response tidak valid. Coba lagi." };
    }

    // 5. Best-effort persist into `generations` (failure here does NOT
    //    block the user from seeing their marketplace description).
    const { error: insertError } = await supabase
      .from("generations")
      .insert({
        user_id: user.id,
        product_id: productId,
        model: result.model,
        module: "marketplace",
        subtype: platform,
        status: "completed",
        input_prompt: prompt,
        result: marketplace as unknown as Json,
        tokens_used: result.tokensUsed,
        duration_ms: result.durationMs,
      });

    if (insertError) {
      console.warn(
        "Failed to save marketplace generation:",
        insertError,
      );
    }

    return { data: marketplace };
  } catch (err) {
    console.error("generateMarketplaceDescription error:", err);
    return {
      error:
        err instanceof Error
          ? err.message
          : "Gagal generate marketplace description",
    };
  }
}
