...
/** Per-platform display name for the tiny tag on each hook card. */
...
/**
 * Hook Generator module — fetches the user's saved products, lets them
 * pick one + set platform/tone/audience, then calls the `generateHooks`
 * server action to produce 3-5 hook variations. Results render in a
 * right-hand panel with one card per hook, each with a copy-to-clipboard
 * shortcut.
 *
 * States covered:
 ...
 * - form submitting — spinner + "Menghasilkan hook ideas..."
 * - form error — error block + "Coba Lagi" retry
 * - form success — list of hook cards
 */
export function HookGenerator() {
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      ...
      if (fetchError) {
        console.error("HookGenerator: load products error:", fetchError);
   ...
   * validates with Zod (see `lib/validation/hook.ts`) and returns
   * `{ data?: HookResult[]; error?: string }`.
   *
  ...
  async function runGeneration() {
    ...
    setResults(hooks);
    toast.success("Hook ideas berhasil dibuat!");
  }
  ...
  /**
   * Copy a single hook to the clipboard and flip the button label for
   * `COPY_FEEDBACK_MS`. Falls back to a no-op if the Clipboard API is
   * unavailable (e.g. on http://, very old browsers).
   */
  async function handleCopy(hookText: string, index: number) {
    try {
    ...
    <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
      ...
      <Card className="md:col-span-1">
        <CardHeader>
          ...
          <CardDescription>
            Pilih produk dan atur target audiens hook Anda.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form className="flex flex-col gap-4" onSubmit={handleGenerate}>
            <div className="flex flex-col gap-2">
              <Label htmlFor="hook-product">Produk</Label>
              <select
                id="hook-product"
            ...
            <div className="flex flex-col gap-2">
              <Label htmlFor="hook-platform">Platform</Label>
              <select
                id="hook-platform"
                value={platform}
            ...
            <div className="flex flex-col gap-2">
              <Label htmlFor="hook-audience">Target audience</Label>
              <Input
                id="hook-audience"
                value={audience}
      ...
      <Card className="md:col-span-2">
        <CardHeader>
          ...
          <CardDescription>
            Hook ideas yang dihasilkan AI. Klik tombol &ldquo;Salin&rdquo;
            untuk menyalin.
          </CardDescription>
        </CardHeader>
      ...
      return (
        <ul className="flex flex-col gap-3">
          {results.map((hook, index) => {
            ...
            const platformLabel =
              PLATFORM_BADGE[hook.platform as Platform] ?? hook.platform;
            return (
              <li
                key={`${hook.text}-${index}`}
              ...
              >
                ...
                <p className="mb-2 text-base font-medium leading-relaxed text-indigo-950">
                  {hook.text}
                </p>
                {hook.note ? (
    ...
    return (
      ...
      >
        ...
        <p className="max-w-sm text-sm text-slate-500">
          ...
          3-5 hook ideas yang siap pakai.
        </p>
      </div>
    );
  }
